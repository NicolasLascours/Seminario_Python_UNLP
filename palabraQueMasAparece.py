...
limpio = set(texto.lower().split())
mas_ocurrencias = 0

for elem_primero in limpio:
    cant = 0
    for elem_segundo in original:
        if elem_primero == elem_segundo:
            cant += 1
            if cant > mas_ocurrencias:
                palabra = elem_primero
                mas_ocurrencias = cant
       
#print(f"la palabra {palabra} con {mas_ocurrencias} fue la de mas ocurrencias")


print(Counter(original).most_common()[0]) # al metodo counter le paso una lista como parametro 
#y luego le envio el mensaje de mas comun y que imprima la posición 0
